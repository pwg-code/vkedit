...
export class AlignElementsCommand extends BaseCommand {
  ...
  constructor(
    ...
    this.elementIds.forEach((id) => {
      const element = this.host.getPlugin('graphic-registry-plugin')?.getElement(id)
      if (element) {
  ...
  private alignElements(): void {
    const elements = this.elementIds
      .map((id) => this.host.getPlugin('graphic-registry-plugin')?.getElement(id))
    ...
    switch (this.alignment) {
      case 'left':
        ...
        elements.forEach((el) => {
          el.x = minX
          this.getElementNode(el.id)?.x(el.x)
        })
        break
      ...
      case 'top':
        ...
        elements.forEach((el) => {
          el.y = minY
          this.getElementNode(el.id)?.y(el.y)
        })
        break
      ...
      case 'centerX':
        ...
        elements.forEach((el) => {
          el.x = centerX - (el as any).width / 2
          this.getElementNode(el.id)?.x(el.x)
        })
      ...
      case 'centerY':
        ...
        elements.forEach((el) => {
          el.y = centerY - (el as any).height / 2
          this.getElementNode(el.id)?.y(el.y)
        })
        break
    }
